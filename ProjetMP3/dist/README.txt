...
Ce serait bien d'avoir une interface GUI du côté client et de séparer le contrôle du serveur du player MP3.
...
pour lister et jouer la musique.
    ...
    Ainsi, on à accès aux fonctionnalités :
        1 - Ajouter une chanson. Il faut donner l titre, artiste et le path complet vers le fichier mp3.
    ...
    Le path des fichiers doit être complet, depuis la racine, afin que le serveur puisse accéder sans problèmes. Sinon,
    c'est possible aussi de donner un path relatif au dossier où se trouvent les sources du serveur.
    Pour simplifier les tests, dans <path vers le contenu du ZIP>/src/serveur/mp3 il y a deux fichiers mp3 appelés
