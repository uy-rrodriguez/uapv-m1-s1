...
class ABR<T> {
    ...
    */
    ...
    private static void assignerFils(ABR pere, ABR fils, boolean aGauche) {
    	if (aGauche)
    ...
    /* Rotations qui modifient la donnee a la racine sans deplacer la racine
     * 
	    public void rotationGauche() {
	    	ABR<T> r = new ABR<>(this.donnee);
	    	ABR<T> D = this.filsDroit;
	    	
	    	assignerFils(r, D.filsGauche, false);
	    	assignerFils(r, this.filsGauche, true);
	    	
	    	// D passe a la racine
	    	this.donnee = D.donnee;
	    	assignerFils(this, D.filsDroit, false);
	    	assignerFils(this, r, true);
	    }
	    
	    public void rotationDroite() {
	    	ABR<T> r = new ABR<>(this.donnee);
	    	ABR<T> G = this.filsGauche;
	    	
	    	assignerFils(r, G.filsDroit, true);
	    	assignerFils(r, this.filsDroit, false);
	    	
	    	// G passe a la racine
	    	this.donnee = G.donnee;
	    	assignerFils(this, G.filsGauche, true);
	    	assignerFils(this, r, false);
	    }
    */

    public void rotationGauche() {
    	//System.out.println("rotationGauche");
    	ABR<T> D = this.filsDroit;
    	
    	assignerFils(this, D.filsGauche, false);
    	
    	// D passe a la racine
    	assignerFils(D, this, true);
    	D.pere = null;
    }
    
    public void rotationDroite() {
    	//System.out.println("rotationDroite");
    	ABR<T> G = this.filsGauche;
    	
    	assignerFils(this, G.filsDroit, true);
    	
    	// G passe a la racine
    	assignerFils(G, this, false);
    	G.pere = null;
    }
    
    public void rotationGaucheDroite() {
    	//System.out.println("rotationGaucheDroite");
    	this.filsGauche.rotationGauche();
    	this.rotationDroite();
    }
    
    public void rotationDroiteGauche() {
    	//System.out.println("rotationDroiteGauche");
    	this.filsDroit.rotationDroite();
    	this.rotationGauche();
    }
    
    /**
     * Soit T un arbre binaire de racine r, on appelle déséquilibre de T la diférence
     * 		d(r) = h(G) − h(D)
     * où h(G) (resp. h(D) ) désigne la hauteur du sous-arbre de racine r.fg (resp. r.fd).
     * 
     * @return
     */
    public int desequilibre() {
    	int hG = (this.filsGauche == null ? 0 : this.filsGauche.hauteur());
    	int hD = (this.filsDroit == null ? 0 : this.filsDroit.hauteur());
    	return hG - hD;
    }
    
    public void reequilibrage() {
    	int dr = this.desequilibre();
    	
    	if (dr == 2) {
    		int dfg = this.filsGauche.desequilibre();
    		
    		// Test de reequilibre du fils
    		if (dfg == 2 || dfg == -2) {
    			this.filsGauche.reequilibrage();
    			this.filsGauche = this.filsGauche.pere;
    		}
    		
    		dfg = this.filsGauche.desequilibre();
    		if (dfg == 1)
    			this.rotationDroite();
    		else if (dfg == -1)
    		...
    		int dfd = this.filsDroit.desequilibre();
    		
    		// Test de reequilibre du fils
    		if (dfd == 2 || dfd == -2) {
    			this.filsDroit.reequilibrage();
    			this.filsDroit = this.filsDroit.pere;
    		}
    		
    		dfd = this.filsDroit.desequilibre();
    		if (dfd == -1)
    			this.rotationGauche();
    		else if (dfd == 1)
    ...
    public void insertionFeuilleEquilibre(DonneeGenerique d) {
